...
<option  value="Male" v-if=" numRows > 0">Male </option>
<option  value="Female" v-if=" numRows > 0">Female </option>
</select>
...
<option v-for="q in qualification" v-bind:value="q.id">{{ q.qualification }} </option>
...
<option v-for="x in tablequalification"  v-bind:value="x.id">  {{ x.qualification }} </option>
</select>
</div>
</div>
</div>
...
<div id="table"  v-if="numRows > 0">
    <div style="overflow: auto;width:100%;overflow-y: hidden;">
        ...
        <table class="table" id="table1" style="padding:0px;margin:0px;">
            ...
            <div style="padding:7px;border-top:solid thin rgba(0, 0, 0, 0.125);border-left:solid thin rgba(0, 0, 0, 0.125)">Males</div>
            <div style="padding:7px;border-top:solid thin rgba(0, 0, 0, 0.125);border-left:solid thin rgba(0, 0, 0, 0.125)">Females</div>
            </td>
            ...
            <div style="padding:7px;border-top:solid thin rgba(0, 0, 0, 0.125);border-left:solid thin rgba(0, 0, 0, 0.125)">{{ item3.males }}</div>
            <div style="padding:7px;border-top:solid thin rgba(0, 0, 0, 0.125);border-left:solid thin rgba(0, 0, 0, 0.125)">{{ item3.females }}</div>
...
<div style="padding:7px;color:rgb(244, 246, 246);background:rgb(244, 246, 246);">
{{ item2.name.toUpperCase()}}
</div><div v-if="genderName==='Male'" style="padding:7px;border-top:solid thin rgba(0, 0, 0, 0.125);border-left:solid thin rgba(0, 0, 0, 0.125)">Males</div>
<div v-else-if="genderName==='Female'" style="padding:7px;border-top:solid thin rgba(0, 0, 0, 0.125);border-left:solid thin rgba(0, 0, 0, 0.125)">Females</div>
</td>
<td v-for="item3 in tableTotals" v-if="item2.id===item3.id" style="width:100px;padding:0;">
<div style="padding:7px;background:rgb(244, 246, 246);font-weight:bold;">
{{item3.totals}}
</div><div v-if="genderName==='Male'" style="padding:7px;border-top:solid thin rgba(0, 0, 0, 0.125);border-left:solid thin rgba(0, 0, 0, 0.125)">{{ item3.males }}</div>
<div v-else-if="genderName==='Female'" style="padding:7px;border-top:solid thin rgba(0, 0, 0, 0.125);border-left:solid thin rgba(0, 0, 0, 0.125)">{{ item3.females }}</div>
</td>
...
<h6 class="nk-block-title" style="padding-bottom:20px;"> Teachers by  Qualification and Region<span style="padding-left:5px;">{{ this.$store.state.enrolment_year }}</span> </h6>
...
var session=this.$session.get('userData');
const searchData = {
    region:this.selectedRegion,district:this.selectedDistrict,year:this.selectedYear,qualificationId:this.qualificationId};
const headers = {
'Authorization':'Bearer '+session
};
...
this.axios.post('/api/filter/primary/teacher/gender/qualification/region',searchData,{headers})
.then(response => {
var status=response.status;
this.status=response.status;
if(status===200){
...
this.axios.post('/api/primary/teacher/gender/qualification/region/advanced-filter',searchData,{headers})
.then(response => {
var status=response.status;
this.status=response.status;
if(status===200){
...
this.axios
.get('/api/primary/teacher/gender/qualification/region',{headers})
.then(response =>{
this.status=response.status;
