...
<li id="hideInput">
<select  class="form-control" id="default-01" placeholder="Input placeholder" style="width:180px;padding:6px" v-model="selectedGender">
...
<li id="hideInput">
<select class="form-control" id="default-01" placeholder="Input placeholder" style="width:180px;padding:6px" v-model="selectedYear">
...
<a href="javascript:void(0);" class="btn" style="background:#1CA1C1;color:white"  @click="filterpayroll()">Filter</a></li>
...
<span class="sub-text">
    <a href="javasript:void(0)" class="btn btn-primary" data-dismiss="modal" aria-label="Close"    v-if="toYear!==''&& fromYear!==''&& note==='' " @click="advancedfilter()">Submit</a>
...
<tbody  style="border:none;" v-for="item1 in tablepayroll">
...
this.axios.post('/api/primary/school/teacher/payroll/advanced-filter',searchData,{headers})
.then(response => {
...
this.tablepayroll=response.data.payroll;
...
//filter api
filterpayroll:function(){

  if(this.selectedGender!==''
  || this.selectedYear!==''){
    this.filterLoading=true;
     var session=this.$session.get('userData');
    const searchData = {gender:this.selectedGender,year:this.selectedYear};
    const headers = {
    'Authorization':'Bearer '+session
    };
this.axios.post('/api/filter/primary/school/teacher/payroll',searchData,{headers})
.then(response => {
var status=response.status;
this.status=response.status;
if(status===200){
...
this.selectedGender='';
 ...
 this.dataLoadingdd=true;
    ...
    this.axios
.get('/api/primary/school/teacher/payroll',{headers})
.then(response =>{
this.status=response.status;
...
this.tableFemale=response.data.females;
this.grandTotal=response.data.grand_total;
this.numRows=response.data.numRow;
this.chartData=response.data.chart;
this.allYear=response.data.filterYears;
this.$store.state.reportTitle='BY PAYROLL STATUS';
this.$store.state.enrolment_year=response.data.minYear+' - '+response.data.maxYear;
this.back=false;
$('#table2').hide();
$('#table1').show();
}
