...
<li><a style="cursor:pointer;" @click="mainMenu('table1','By School Ownership')">Primary School Readers By School Ownership</a></li>
...
<h6 class="nk-block-title" style="padding-bottom:20px;text-transform: capitalize;">Primary Primary School Readers by School Ownership <span style="padding-left:5px;">{{ this.$store.state.enrolment_year }}</span></h6>


<column-chart :data="chartData" :stacked="true"  :dataset="{borderWidth: 0}" :download="true" xtitle="Subjects" ytitle="Number of Readers" height="600px" style="padding-top:10px;"
></column-chart>
...
this.axios.post(
'/api/primary/school/readers/advanced-filter',
filterData,
{headers})
.then(
response=>{
...
this.axios.post(
'/api/primary/school/readers/filter',
filterData,
{headers})
.then(
response=>{
...
this.axios
.get('/api/primary/school/readers',{headers})
.then(response =>{
this.status=response.status;
if(response.status===200){
