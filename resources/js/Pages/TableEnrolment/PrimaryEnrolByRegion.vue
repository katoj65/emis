...
<li><a style="cursor: pointer;" @click="mainMenu('table7','By  Class and  Year ')">Enrolment by Class and  Year</a></li>
 ...
 <li id="filterShow" style="cursor: pointer;padding :6px;">
     <em class="icon ni ni-filter-alt" style="color: #000;font-size: 18px;" ></em>
 </li>
<li id="hideInput">
<select  @change="changefilterRegion" class="form-control"  style="width:138px;padding :6px;" v-model="selectedRegion">
<option  value="">Select Region</option>
<option v-for="n in allRegions" v-bind:value="n.id">{{ n.name }}</option>
</select>
</li>
<li id="hideInput">
<select  class="form-control" id="default-01" placeholder="Input placeholder" style="width:126px;padding :6px;" v-model="selectedDistrict">
...
<li id="hideInput">
<select class="form-control" id="default-01" placeholder="Input placeholder" style="width:120px;padding :6px;" v-model="selectedYear">
...
<span v-if="fromYear!=='' && toYear!=='' && note===''" @click="advancedFilter" style="cursor:pointer;">
...
<td v-for="item3 in tableBody" v-if="item2.id===item3.district_id">
<span v-for="sm in item3.sum">{{sm}}</span>
</td>
...
<h6 class="nk-block-title" style="padding-bottom:20px;">PRIMARY SCHOOL ENROLMENT BY REGION <span style="padding-left:5px;">{{ this.$store.state.enrolment_year }}</span> </h6>
...
var session=this.$session.get('userData');
const searchData = {fromYear:this.fromYear,toYear:this.toYear};
const headers = {
'Authorization':'Bearer '+session
};
...
this.axios.post('/api/primary/enrolment/region/advanced-filter',searchData,{headers})
.then(response => {
var status=response.status;
this.status=response.status;
if(status===200){
...
this.tableGrandTotal=response.data.grandTotal;
this.tableYears=response.data.years;
this.allFilteryear=response.data.allFilter;
this.chartData=response.data.tableChart;
