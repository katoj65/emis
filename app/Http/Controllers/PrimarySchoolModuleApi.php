...
class PrimarySchoolModuleApi extends Controller
{
    /**
...
$response[]=array(
$s->subject,
PrimaryTextbooksModel::select('books')
->where('year',$year)
->where('founder',$f->id)
->where('subject',$s->id)
->sum('books')
...
$count=count(PrimaryTextbooksModel::get());
if($count>0){
...
'id'=>$s->id,
'year'=>$enrolment_year,
'item'=>number_format(PrimaryTextbooksModel::select('books')->where('subject',$s->id)->where('year',$enrolment_year)->where('founder',$f->id)->sum('books'))
);
}
}
...
if($select_subject){
    ...
    $year_total[]=array(
        'year'=>$yy->school_year,
        'fid'=>$f->id,
        'item'=>number_format( PrimaryTextbooksModel::select('books')
